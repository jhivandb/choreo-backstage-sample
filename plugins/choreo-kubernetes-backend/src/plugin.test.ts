...
import { choreoKubernetesPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [choreoKubernetesPlugin],
    });

    await request(server).get('/api/choreo-kubernetes/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/choreo-kubernetes/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/choreo-kubernetes/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/choreo-kubernetes/todos/${createdTodoItem.id}`)
